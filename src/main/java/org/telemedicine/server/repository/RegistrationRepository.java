package org.telemedicine.server.repository;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.stereotype.Repository;
import org.telemedicine.server.entity.Registration;

import java.time.LocalDate;
import java.util.List;
import java.util.Optional;

@Repository
public interface RegistrationRepository extends JpaRepository<Registration, String> {
    Registration findByPatientsId(String patientId);
    List<Registration> findByDate(LocalDate date);
    @Query(value = "SELECT r FROM Registration r WHERE r.patients.id = :patientsId ORDER BY r.date DESC LIMIT 1")
    Optional<Registration> findFirstByPatientsIdAndDateDesc(String patientsId);
}
